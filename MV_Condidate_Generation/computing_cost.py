...
def ComputeCost(MVPP,
    ...
    List_nodes = [n for n in List_nodes if len(list(MVPP.successors(n))) != 0 and len(list(MVPP.predecessors(n))) != 0]
    for node in List_nodes:  # for each inner node
        #print("potential view ", node)
        ...
        if len(node_contains) > 1:  # check if a node is a join node at least two tables
            ...
            for id_table in node_contains:  # for each table in the node , table id =0 is the fact table
                ...
                Table_Name = [key for key, value in DW_schema().items() if value == id_table_without_X][0]
                tables_names_for_sql_query = [key for key, value in DW_schema2().items() if value == id_table_without_X][0]
