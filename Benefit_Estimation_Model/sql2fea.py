...
import numpy as np
from Benefit_Estimation_Model.PGUtils import pgrunner
from Benefit_Estimation_Model.JOBParser import TargetTable,FromTable,Comparison
...
class Sql2Vec:
    ...
    def to_vec(self,sql):
        ...
        # parse_dict = DictionaryParser().parse_dict
        self.sql = sql
        import time
        # startTime = time.time()
        parse_result = parse_dict(self.sql)[0]["SelectStmt"]
        print( parse_result["fromClause"])
...
def get_plan_stats(data):
   # print(data)
    return [value_extractor.encode(data["Total Cost"])]
...
class TreeBuilder:
    ...
    def __featurize_join(self, node):
        ...
        feature = np.concatenate((arr,padding, self.__stats(node)))
    ...
    def __featurize_scan(self, node):
        ...
        table_name = np.zeros(1)
        table_name[0] = value_extractor.encode(self.__alias_name(node))
        arr = np.zeros(len(ALL_TYPES))
        arr[ALL_TYPES.index(node["Node Type"])] = 1
        feature = np.concatenate((arr,table_name, self.__stats(node)))
        feature = torch.tensor(feature,device = config.device,dtype = torch.float32).reshape(-1,config.input_size)
        # return (feature,torch.tensor(self.__alias_name(node),device = config.device,dtype = torch.long))
        return feature
    def plan_to_feature_tree(self, plan):
        ...
        if is_join(plan):
            ...
            #print('is_join',my_vec, left, right)
